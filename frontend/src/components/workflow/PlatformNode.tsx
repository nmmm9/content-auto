...
export default function PlatformNode({ id, data }: NodeProps) {
  ...
  return (
    <div
      ...
      className={`px-5 py-4 rounded-xl border-2 shadow-lg bg-white ${borderByStatus[d.status || 'idle'] || 'border-gray-200'} ${d.isMain ? 'ring-2 ring-red-500 ring-offset-2 min-w-[300px]' : 'min-w-[250px]'} ${d.isMain && d.analysisResult && (d.status === 'ready' || d.status === 'success') ? 'cursor-pointer hover:shadow-xl transition-shadow' : ''}`}>
      ...
      {d.status === 'waiting_approval' && d.generatedContent && (
        <div className="mb-3 p-2 bg-gray-50 rounded-lg text-xs">
          {d.generatedContent.title && (
            <div className="font-medium text-gray-900 truncate">{d.generatedContent.title}</div>
          )}
          {d.generatedContent.caption && (
            <div className="text-gray-600 truncate mt-1">{d.generatedContent.caption}</div>
          )}
          {d.generatedContent.hashtags && d.generatedContent.hashtags.length > 0 && (
            <div className="text-blue-500 truncate mt-1">
