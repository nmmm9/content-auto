// 분석·변환 프롬프트 정의 (backend/app/services/video_analyzer.py에서 TS로 이식)
...
export const ANALYSIS_PROMPT = `당신은 유튜브 영상 콘텐츠 분석 전문가입니다.
...

{
  ...
  "detailed_summary": "영상의 처음부터 끝까지 시간 순서대로 전체 내용을 상세히 정리. 각 파트/챕터별로 무슨 내용을 다루는지, 어떤 장면이 나오는지, 출연자가 어떤 말을 하는지 구체적으로 서술",
...
const CONTEXT_BLOCK_LONG = `${CONTEXT_BLOCK}
...
export const PLATFORM_PROMPTS: Record<string, PromptConfig> = {
  youtube_shorts: {
...
/** {변수} 치환 — JSON 예시의 중괄호나 미지의 토큰에서 죽지 않는다 */
...
export function buildFormatData(
  analysis: Record<string, unknown>,
  videoTitle = '',
  videoUrl = '',
  brandVoice = ''
): Record<string, string> {
  const str = (key: string) => String(analysis[key] ?? '')
  const list = (key: string) =>
    Array.isArray(analysis[key]) ? (analysis[key] as unknown[]).map(String) : []
  ...
  return {
    ...
    key_points: list('key_points').map((p) => `- ${p}`).join('\n'),
    scenes: list('scenes').map((s) => `- ${s}`).join('\n'),
    audio_summary: str('audio_summary'),
    on_screen_text: str('on_screen_text') || '없음',
    visual_style: str('visual_style'),
    recommended_style: str('recommended_style'),
    viral_hook: str('viral_hook'),
