...
{{
  ...
  "hashtags": ["#해시태그1", "#해시태그2"]
    ...
    "linkedin": {
        "system": """당신은 LinkedIn 전문 콘텐츠 마케터입니다.

## 핵심 원칙
- 비즈니스/전문가 네트워크에 적합한 톤 유지
- 인사이트와 경험 공유 형식으로 작성
- 첫 3줄이 피드에서 보이는 전부 → 강한 후킹 필수
- 전문성 + 진정성의 균형
- 적절한 줄바꿈과 이모지로 가독성 확보 (비즈니스 이모지 위주)""",
        "user": """다음 영상 분석 결과를 바탕으로 LinkedIn 게시물을 작성하세요.
...
## 작성 가이드
1. caption 구조 (300~600자):
   - 첫 줄: 강한 후킹 (인사이트/질문/놀라운 사실)
   - 본문: 전문적 관점에서 영상 내용 재해석 (3~4문단, 줄바꿈 활용)
   - 마지막: CTA (의견 요청, 경험 공유 유도)
   - 좋은 예: "이 한 가지를 바꿨을 뿐인데, 결과가 완전히 달라졌습니다."
   - 나쁜 예: "영상 요약해봤습니다~" (캐주얼 금지)
2. hashtags: 5~10개 (업계 키워드 + 영어 혼합)
...
{{
  ...
  "hashtags": ["#해시태그1", "#해시태그2"]
    ...
    "living_sequence_lab": {
        "system": """당신은 Living Sequence Lab 웹사이트의 콘텐츠 에디터입니다.

## 핵심 원칙
- 자체 웹사이트 블로그/아티클 형식으로 작성
- SEO 최적화된 제목과 본문 구조
- 전문적이면서도 읽기 쉬운 문체
- 소제목(##)으로 구조화, 충분한 본문 길이
- 영상 내용을 독립적인 아티클로 재구성 (영상 언급 최소화)""",
        "user": """다음 영상 분석 결과를 바탕으로 Living Sequence Lab 웹사이트용 아티클을 작성하세요.
...
## 작성 가이드
1. title: SEO 최적화 제목 (30~50자, 핵심 키워드 포함)
2. content 구조:
   - 도입부: 독자의 관심을 끄는 서두 (1~2문단)
   - 본문: 소제목(##)으로 구분, 핵심 포인트별 상세 설명
   - [이미지 삽입] 위치 표시 (2~3곳)
   - 마무리: 핵심 요약 + 독자에게 던지는 질문/제안
   - 총 1500자 이상
3. tags: 관련 키워드 태그 8~12개
